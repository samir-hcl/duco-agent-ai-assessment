...
function PreAuthLetterCard({ letter }: { letter: PreAuthLetter }) {
  ...
  const downloadPdf = () => {
    // Generate a self-contained HTML document as a downloadable PDF-ready file
    const htmlContent = `<!DOCTYPE html><html><head><title>Pre-Auth Letter — ${letter.patientName}</title>
      <style>
        @page { size: A4; margin: 20mm; }
        body{font-family:'Segoe UI',Arial,sans-serif;max-width:700px;margin:40px auto;padding:20px;color:#1a1a1a;line-height:1.7;font-size:13px}
        ...
        .codes{margin:10px 0;padding:8px;background:#f8f9fa;border-radius:4px;font-size:11px}
      ...
      <div class="meta">Patient: ${letter.patientName} | Insurer: ${letter.insurerName} | Plan: ${letter.planName} | Generated: ${new Date(letter.generatedAt).toLocaleDateString()}</div>
      <div class="codes">CPT: ${letter.procedures.map(p => p.code).join(', ')} | ICD-10: ${letter.diagnoses.map(d => d.code).join(', ')} | Est. Cost: ${fmt(letter.estimatedCost)}</div>
      <pre>${letter.content}</pre>
      <div class="footer">Generated by DuCO-Agent • Coordination of Benefits AI System • ${new Date().toISOString()}</div>
      </body></html>`;
    const blob = new Blob([htmlContent], { type: 'application/pdf' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `pre-auth-${letter.patientName.replace(/\s/g, '_')}-${letter.letterType}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
  };
